...
function generateProceduralStory(world) {
    const name = world.name;
    const year = world.currentYear;
    const era = world.currentEra;
    const speciesNames = world.species.map((s) => s.name).join(", ") || "various organisms";
    // Group events
    const wars = world.history.filter((h) => h.eventType === "WAR");
    const alliances = world.history.filter((h) => h.eventType === "ALLIANCE");
    const techAdvances = world.history.filter((h) => h.eventType === "TECHNOLOGY_ADVANCED");
    const disasters = world.history.filter((h) => h.eventType === "DISASTER");
    ...
    if (techAdvances.length > 0) {
        story += `As intelligence sparked, civilizations began to form. Tech advancements were documented across the galaxy: ${techAdvances.slice(0, 3).map((t) => t.description).join(", ")}. What started as simple tribal tools soon laid the foundations for complex kingdoms and spacefaring empires.\n\n`;
    }
    else {
    ...
    if (disasters.length > 0) {
        story += `*Historical Note on Cataclysms:* The galaxy was tested by severe cataclysms, including: ${disasters.slice(0, 2).map((d) => d.description).join(", ")}, which shook the civilizations to their core.\n\n`;
    }
